...
sub _decode {
  ...
  foreach my $buf ($_[-1]) {
    ...
    foreach my $op (@{$ops}) {
      ...
      if (length $op->[cTAG]) {
        #print "Trying op with tag=".ord($op->[cTAG]). ", type=". $op->[cTYPE]." and ext=". ($op->[cEXT]||"<unset>")." at $pos (..$end)\n";
        ...
        TAGLOOP: {
          ...
          #print "Read tag: ".ord($tag)."[$npos:+$len ($indef)]\n";

          if ($tag eq $op->[cTAG]) {
            #print "Matching tag, decoding primitive\n";
          ...
          {
            #print "Matching constructing tag (".$op->[cTYPE]."/".@ctr."), recurse\n";
            my $inner_op = [@$op];
        ...
        if ($op->[cTYPE] == opANY) {
          #print "Trying ANY with ext=". $op->[cEXT]." and define=".($op->[cDEFINE]||"<unset>")." at $pos (..$end)\n";
          ...
          #print "Trying CHOICE with ext=". $op->[cEXT]." at $pos (..$end)\n";
          ...
          #print "Trying EXTENSIONS with ext=". $op->[cEXT]." at $pos (..$end)\n";
